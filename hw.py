...
class AbstactViewConsol(ABC):
    @abstractmethod
    def show_text(self, text):
        pass


class ViewConsol(AbstactViewConsol):
    def show_text(self, text):
        printing_text = text if text else 'Text not found'
        print(printing_text)
...
def main():
    ...
    interface = ViewConsol()
    interface.show_text("Welcome to the assistant bot!")
    while True:
        user_input = input("Enter a command: ")
        command, *args = parse_input(user_input)

        if command in ["close", "exit", '0']:
            interface.show_text("Good bye!")
            with open(file_path, "wb") as file:
                pickle.dump(book, file)
            break

        elif command == "hello":
            interface.show_text("How can I help you?")

        elif command == "add":
            interface.show_text(add_contact(args, book))

        elif command == "change":
            interface.show_text(change_contact(args, book))

        elif command == "phone":
            interface.show_text(show_phone(args, book))

        elif command == "all":
            interface.show_text(show_all(book))

        elif command == "add-birthday":
            add_birthday(args, book)

        elif command == "show-birthday":
            interface.show_text(show_birthday(args, book))

        elif command == "birthdays":
            birthdays = book.get_upcoming_birthdays()
            if not len(birthdays):
                interface.show_text("There are no upcoming birthdays.")
                continue
            for day in birthdays:
                interface.show_text(f"{day['name']} - {day['congratulation_date']}")

        else:
            interface.show_text("Invalid command.")
